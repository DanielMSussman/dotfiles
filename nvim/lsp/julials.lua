-- Adapted from the nvim-lspconfig repo:
-- https://github.com/neovim/nvim-lspconfig/blob/master/lsp/julials.lua
-- I had to alter the activate_env function a bit to make it work?

local root_files = { 'Project.toml', 'JuliaProject.toml' }

-- receive opts to reflect the fact that we get a table of options, not just a string...
local function activate_env(opts)
    local path = opts.args -- ... and then get the path string from the opts table.
    ...
    -- if no arg is given, let user select from available envs
    if path and path ~= '' then
        ...
        -- This block now runs correctly when no argument is given
